...
Atlas built-in traceroute measurements. \Cref{fig:hops-per-measument} shows the
...
\Cref{sec:latency-wholerange}. Both histograms have a pattern that spikes at
...
in round trip time from hop to hop. \Cref{fig:latency-change-per-hop-1} and
\Cref{fig:latency-change-per-hop-2} show how the round trip time measured in
...
listed in the appendix (\Cref{fig:latency-change-per-hop-appendix-1} --
\Cref{fig:latency-change-per-hop-appendix-16}).
...
in \Cref{fig:latency-change-per-hop-1-ca}). Therefore, we looked at the
