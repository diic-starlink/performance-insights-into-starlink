...
\Cref{fig:latencies-per-weekday} illustrates the latencies that occur in each
...
minimum latency). The results are shown in \Cref{fig:weekday-statistics}.
...
week. \Cref{fig:latency-per-weekday-1st-2nd-april} shows the latency
...
\Cref{fig:latency-per-weekday-weeks-1} and
\Cref{fig:latency-per-weekday-weeks-2} show the weeks between April 1 and April
28, 2024.
...
among different weekdays (as shown in \Cref{fig:latencies-per-weekday} and
\Cref{fig:weekday-statistics}).
